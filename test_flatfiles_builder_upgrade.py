...
TEMP_FILE = "test_opra_multi_upgrade.csv.gz"
...
def run_multi_test():
    print("\n--- DEBUG MODE: 1M vs 3M Dual-Tenor (RATIO MATH) Test ---")
    ...
    past_dates = pd.date_range(end=today - pd.tseries.offsets.BDay(3), periods=100, freq='B')
    test_dates = random.sample(list(past_dates), 3)
    ...
    print("\n" + "="*100)
    print("--- FINAL MULTI-TICKER / DUAL-TENOR (RATIO) TEST RESULTS ---")
    if results:
