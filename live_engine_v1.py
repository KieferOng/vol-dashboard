...
class DashboardEngine:
    def __init__(self, ticker: str, hist_dir="massive_historical_options_data"):
        self.ticker = ticker.upper()
        self.headers = {"Authorization": f"Bearer {MASSIVE_API_KEY}"}
        self.rfr = self._fetch_risk_free_rate()
...
def build_all_tickers():
    print("Initiating Live Market Pull for 50 Tickers...")
    master_data = []
    
    for i, ticker in enumerate(ALL_TICKERS, 1):
        print(f"[{i}/50] Processing {ticker}...")
        engine = DashboardEngine(ticker)
        data = engine.process()
        master_data.append(data)
        
    df = pd.DataFrame(master_data)
    df.to_csv("live_dashboard_feed.csv", index=False)
    print("\n✅ Success! Master feed saved to 'live_dashboard_feed.csv'.")
    return df
